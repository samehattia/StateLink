...
proc wait_axis_rx_interrupt {} {
	...
	while 1 {
		refresh_hw_vio [get_hw_vios -of_objects [get_hw_devices "xcku040_0"] -filter "CELL_NAME=~memcached_i/StateLink_AXIS_0/vio_0"]
		set interrupt [get_property INPUT_VALUE [get_hw_probes "memcached_i/StateLink_AXIS_0/axi_fifo_mm_s_0_interrupt" -of_objects [get_hw_vios -of_objects [get_hw_devices "xcku040_0"] -filter "CELL_NAME=~memcached_i/StateLink_AXIS_0/vio_0"]]]
	...
	create_hw_axi_txn rd_txn [get_hw_axis hw_axi_1] -address 44A0_0000 -len 1 -type read -force
	run_hw_axi -verbose rd_txn

	# Interrupt Status Register (ISR) @0x00 <-- Input FFFFFFFF: Reset interrupt bits
	create_hw_axi_txn wr_txn [get_hw_axis hw_axi_1] -address 44A0_0000 -len 1 -type write -force -data FFFF_FFFF
	run_hw_axi wr_txn

}

proc read_axis_rx_hw_packet {axis_internal_pipe axis_rx_hw_to_sim_pipe} {
	global AXIS_INTERNAL_PIPENAME AXIS_RX_HW_TO_SIM_PIPENAME AXIS_READ_COMMAND_COUNTER
	...
	while { 1 } {
		...
		create_hw_axi_txn rd_txn_2 [get_hw_axis hw_axi_2] -address 44A0_1000 -len $burst_len -type read -force
...
proc StateLink_AXIS_RX {} {
	global DEVICE_NAME FULL_PROBES AXIS_RX_SIM_TO_HW_PIPENAME AXIS_RX_HW_TO_SIM_PIPENAME AXIS_INTERNAL_PIPENAME AXIS_READ_COMMAND_COUNTER AXIS_RX_HW_THREAD
	...
	create_hw_axi_txn wr_txn [get_hw_axis hw_axi_1] -address 44A0_0018 -len 1 -type write -force -data 0000_00A5
	run_hw_axi wr_txn
	...
	create_hw_axi_txn wr_txn [get_hw_axis hw_axi_1] -address 44A0_0004 -len 1 -type write -force -data 0400_0000
	run_hw_axi wr_txn
